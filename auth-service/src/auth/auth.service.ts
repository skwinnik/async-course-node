...
export class AuthService {
  ...
  async login(loginDto: LoginDto): Promise<JwtDto | null> {
    ...
    const access_token = await this.jwtService.signAsync({ id: user.id, user_name: user.name, role_name: role.name });
    return new JwtDto(access_token);
  }
}
